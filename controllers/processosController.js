...
async function listProcessos(req, res) {
  try {
    ...
    console.error('listProcessos error:', err);
...
async function createProcesso(req, res) {
  try {
    ...
    // Joi já validou, checagem leve por segurança
    if (!clienteId || !numero || !tipo || !status) {
      return res.status(400).json({ error: 'Campos obrigatórios: clienteId, numero, tipo, status' });
    }
    const novo = await model.createProcesso({ clienteId, numero, tipo, status, historico });
    return res.status(201).json(novo);
  } catch (err) {
    console.error('createProcesso error:', err);
...
async function updateProcesso(req, res) {
  try {
    ...
    console.error('updateProcesso error:', err);
...
async function deleteProcesso(req, res) {
  try {
    ...
    console.error('deleteProcesso error:', err);
...
async function getProcesso(req, res) {
  try {
    ...
    console.error('getProcesso error:', err);
